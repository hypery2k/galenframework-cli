node {
  ...
  try {
    ...
    stage('Publish NPM snapshot') {
      def currentVersionCore = sh(returnStdout: true, script: "cd core && npm version | grep \"{\" | tr -s ':'  | cut -d \"'\" -f 4").trim()
      def newVersionCore = "${currentVersionCore}-${buildNumber}"
      sh "cd cli && npm version ${newVersionCore} --no-git-tag-version && npm publish --tag next"
